...
export default function TeamHealthTab({ jobId, apiBase }) {
  ...
  const pages = [
      <div key="risk" className="team-health-tab">
        <p className="card-hint" style={{ marginBottom: "1.5rem" }}>
          Burnout Radar predicts cohort-level burnout risk using an XGBoost classifier based on
          concentration risk, firefighting ratios, and activity pressure.
        </p>
        ...
        >
          ...
          <div style={{ position: "relative", width: "120px", height: "120px" }}>
            ...
            >
              <span style={{ fontSize: "1.5rem", fontWeight: "bold", color: riskColor }}>
                {scorePct}%
              </span>
            </div>
          </div>
      ...
      <div key="drivers" className="team-health-tab">
        <div
          className="team-health-widget"
          style={{
            background: "#1a2332",
        ...
        >
          <h3 style={{ margin: "0 0 1rem 0", fontSize: "1.1rem", color: "#e7ecf3" }}>
              ...
              top_drivers.map((d, i) => (
                <li
                  ...
                  style={{
                    marginBottom: "1rem",
                    paddingBottom: "1rem",
                    borderBottom: i < top_drivers.length - 1 ? "1px solid #2a3548" : "none",
                  }}
                >
                  <div
                    style={{
                  ...
                  <div style={{ fontSize: "0.85rem", color: "#8b9cb3" }}>
                    SHAP Contribution: {d.shap_value > 0 ? "+" : ""}
      ...
      <div key="metrics" className="team-health-tab">
        <div
          className="team-health-widget"
          style={{
            background: "#1a2332",
                ...
                {metrics &&
                  Object.entries(metrics).map(([key, val]) => (
                    <tr key={key} style={{ borderBottom: "1px solid #1e2a3a" }}>
                      <td style={{ padding: "0.75rem", color: "#e7ecf3", fontFamily: "monospace" }}>
                        {key}
                      </td>
  ...
  return (
    <PanelPager pages={pages} resetKey={jobId} ariaLabel="Team health pages" />
  );
}
